...
| `done`        | агент вважає тікет виконаним і відкрив PR; рев'ю та merge вирішує оркестратор |
| `blocked`     | заблоковано (вкажіть причину в тілі тікета)                                   |

> Агент сам переводить тікет у `done`, щойно вважає роботу завершеною (реалізовано за **Обсягом**,
> виконано **Критерії приймання** + PR-чекліст, відкрито PR). Це **не** означає, що зміни прийнято:
> рішення про правки чи merge приймає оркестратор на рев'ю PR. Якщо на рев'ю потрібні зміни —
> оркестратор повертає тікет у `in-progress`.
...
5. Відкрити PR (заголовок — Conventional Commits, напр. `feat(auth): add active-org JWT and switch-org`).
   В описі PR послатися на тікет.
6. Щойно агент вважає тікет виконаним — одразу `status: done`, заповнити `pr:`. Подальше рев'ю та
   merge — за оркестратором (за потреби правок він поверне тікет у `in-progress`).
