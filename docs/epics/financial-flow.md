...
6. **Основний баланс обчислюється у UAH.**
   `balanceUah = donationsUah - expensesUah`, де кожна подія використовує власний історичний курс.
...
Для кожної активної події:

```text
amountUah = round(amount * rate, 2)
signedAmountUah =
  donation -> +amountUah
  expense  -> -amountUah
```
...
```text
expensesUah = sum(expense.amount * expense.rate)
donationsUah = sum(donation.amount * donation.rate)
balanceUah = donationsUah - expensesUah
```

Soft-deleted записи не враховуються.
...
Це не замінює `balanceUah`. Значення різних валют не складаються між собою без конвертації.

### 4.3. Змішані валюти

Якщо витрата була `3000 USD`, а донати надійшли у USD, EUR та UAH, цільовим показником є
`balanceUah`, обчислений за збереженим курсом кожної події. Через різні дати і курси нативний
USD-еквівалент може не дорівнювати нулю навіть тоді, коли UAH-баланс закритий.

UI показує:

- `balanceUah < 0`: **Потрібно зібрати**;
- `balanceUah = 0`: **Баланс закрито**;
- `balanceUah > 0`: **Доступний залишок**.

Порівняння з нулем виконується після округлення до 2 знаків.
...
Оскільки кожен донат має `vehicleId`, будь-який org-level баланс є сумою балансів фінансових подій
усіх автомобілів організації. Розподіл одного донату між кількома авто поза обсягом цього епіку:
для кількох авто створюються окремі donation rows.
...
Глобальне злиття дублікатів, перейменування і контактний профіль донора поза обсягом.
...
- решта валютної, vehicle, audit і soft-delete моделі не змінюється.
...
| Авто      | optional vehicle      | required vehicle      |
...
2. `purchasePrice` у переході `-> paid` залишається інформативним полем історії статусу і не створює
   expense автоматично.
3. Реальна купівля авто фіксується окремою expense у фінансовому журналі.
...
6. Міграції цього епіку створюються поверх фінальної схеми статусного епіку; не редагувати
   незавершені VSF-міграції заднім числом.
...
6. Після перевірки даних видалити FK/column/table.
...
- Автоматичне створення expense з `purchasePrice` статусного переходу.
...
- **Обсяг.** Rename `expense_categories -> financial_categories`; drop expense funding FK/column/index,
  funding source table/enum/contracts/dictionary/seed; створення нових таблиць; стратегія з розд. 12.
- **Критерії приймання.** Expense створюється без funding source; runtime-згадок `fundingSource`
  немає; міграція проходить на цільовому середовищі.
...
- **Критерії приймання.** Create/update/list працюють без funding source; старий request field
  відхиляється strict schema.
- **Залежності.** FIN-4.
...
- **Критерії приймання.** Відображаються лише прямо пов'язані з авто записи; створений з картки
  донат автоматично отримує це `vehicleId`.
- **Залежності.** FIN-13.
...
11. Технічна документація оновлена разом із реалізацією.
