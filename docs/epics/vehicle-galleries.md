...
13. **Перенесення фото зберігає підпис** і додає фото в кінець цільової галереї. Перенесення
...
галереї, організації та автомобілю. Якщо Drizzle-оголошення циклічного FK ускладнює схему, FK можна
додати у фінальній SQL-міграції; залишати невалідоване довільне UUID не можна.
...
5. Upload блокує рядок галереї або використовує еквівалентний atomic check, щоб паралельні запити
   не перевищили 30 активних items.
...
Це дозволяє спискам показувати обкладинку без завантаження всіх галерей і без N+1 запитів.
...
- drag-and-drop або інший наявний патерн для сортування фото;
